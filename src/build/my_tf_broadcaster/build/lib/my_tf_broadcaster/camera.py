...
"""
Суть работы скрипта:
- Захватывает видеопоток с камеры Intel RealSense.
- Обнаруживает ArUco-маркеры в изображении с использованием OpenCV.
- Оценивает положение (позицию и ориентацию) каждого маркера с помощью калибровочных параметров камеры.
- Публикует TF-трансформации для каждого обнаруженного маркера в систему ROS2, позволяя другим узлам получать данные о пространственном расположении маркеров.
- Отображает изображение с визуализацией обнаруженных маркеров и их осей для отладки.
"""
